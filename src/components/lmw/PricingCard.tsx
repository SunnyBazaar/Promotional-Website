...
export function PricingCard({ plan, kind, icon, onPreview }: Props) {
  return (
    <article className="reveal group flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/40 hover:shadow-[0_24px_45px_-18px_rgba(255,122,0,0.35)] sm:rounded-3xl sm:p-6">
      <div className="grid gap-3 sm:flex sm:items-start sm:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 className="font-bold leading-snug">{plan.title}</h3>
